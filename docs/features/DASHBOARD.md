# 🛠️ SpaceLens Blueprint: Premium Dark-Theme Dashboard (v2)

## 1. Objective
Refactor the main Dashboard page (`src/features/Dashboard/Dashboard.jsx`) to implement a specific premium, dark-themed grid layout. This revision splits the top analytical bar, integrates two distinct chart types (hourly traffic and daily revenue), and adds detailed zone ranking and performance tables. The entire UI must strictly mirror the aesthetic "Vibe" of the provided reference image (dark slate, high contrast, clean card borders).

## 2. UI/UX Guidelines & Theming (Replicating the Vibe)
- **Theme:** Ultra-Dark Slate/Gray palette (SaaS Premium aesthetic).
- **Background:** `bg-slate-950` or `bg-gray-950`.
- **Card Styling:** All component containers must be distinct "cards" with `rounded-2xl`, `bg-slate-900` or `bg-gray-900`, and a subtle, crisp border `border border-slate-800`. Large internal padding (`p-6` or `p-8`).
- **Layout:** Standard CSS Grid or 12-column grid system with large gaps (`gap-6`) between cards.
- **Typography:** DM Sans (implied premium font). High contrast: `text-white` for values, `text-slate-400` for titles. For numerical data in tables/charts, use a mono font like DM Mono.
...
- Wrap the entire content in a main container using `bg-slate-950 p-6 min-h-screen`.
...
- Each KPI Card structure: Lucide icon (e.g., green `emerald-400` accent), title (`text-slate-400`), prominent value (`text-5xl text-white font-bold`), and a small percentage trend indicator.
...
- Axis: X-axis shows Dates ('14 Mar', '15 Mar', '16 Mar'). Y-axis shows "Currency (VNĐ)".
...
- Distinct `border border-slate-800` on every card block.
